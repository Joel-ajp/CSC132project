
// Function which populates the plant section of the Garden page
window.onload = function populateGarden() {
    let plantCounter = 0;
    let data = JSON.parse(localStorage.getItem("myData"));
    if (data == null) plant.textContent = "You must add plants to your Garden!"
    else {
        data.forEach(e => {
            let dataInfo = "";
            Object.keys(e).forEach(es => dataInfo += e[es] + "<br />")
            plantCounter ++;
            plant.innerHTML += `<div id=${plantCounter} class="plant">
                <img src="data:image/png;base64," alt="Plant">
                <p id="gardenPlants", class="gardenPlantStyle">
                    ${dataInfo}
                    </p>
                    <button class="rmbtn" onclick="removePlantFromGarden(${plantCounter})">Remove</button>
            </div>`
                });
    }
    };

// Function to remove plant from local storage and Garden page
function removePlantFromGarden(plantCounter) {
    //Removes plant from localstorage
    
    // for (let i = 0; i < localStorage.length; i++){
    //  key = localStorage.key(i);
    //  let value = localStorage.getItem(key);
    //  console.log(key, value);
    //}


    //Removes the plant from the page
    localStorage.removeItem(`id: ${plantCounter}`)
    toDel = document.getElementById(plantCounter)
    toDel.remove()
}
