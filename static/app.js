...
function renderPlants(){
    PLANTS.forEach(plant => {
        ...
        <div class="plant">
            ...
            <button class="btn" id="${plant.id}" onclick="addToGarden(${plant.id})">Add to Garden</button>
          </div>

          <div id="${plant.name}" class="modal">
            <!-- Modal content -->
            <div class="modal-content">
                <span class="close" onclick="closeModal('${plant.name}')">&times;</span>
                <h1 id="plantName">${plant.name}</h1>
                <div class="breaker"></div>
                <p>${plant.fullDes}</p>
                <p id="maxTemp">${plant.maxTemp}</p>
                <p id="minTemp">${plant.minTemp}</p>
                <p id="soilMois">${plant.soilMois}</p>
            </div>
          </div>
          `
    });
